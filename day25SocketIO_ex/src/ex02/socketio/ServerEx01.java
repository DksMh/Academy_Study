...
public class ServerEx01 {
	...
	public ServerEx01() {
		...
		PrintWriter pw = null; // -> 어차피 콘솔에 찍을 거니까.
		Scanner scan = new Scanner(System.in);
		try {
			// ServerSocket을 생성하고 
			listener = new ServerSocket(9998); // 모니터 - url의 제일 끝단( End Pointer)
...
//			bw = new BufferedWriter(new OutputStreamWriter(socket.getOutputStream()));
			...
			// 클라이언트의 userId를 읽어오기
			try {
				// 라인의 '\n'이다. '\n'이 없이는 데이터를 reaLine()로 읽을 수 없다
				String userId = br.readLine(); 
				System.out.println("서버 >>> "+userId+"님이 접속 하였습니다!");
			} catch (Exception e) {
				System.out.println("읽어드릴 데이터가 없다.");
			}
